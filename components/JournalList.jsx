...
const JournalList = ({ entries, isEditing }) => {
  return (
    <div className="w-[500px] ">
      {entries.map((entry, i) => (
        <div
          key={i}
          className={`w-full p-8 rounded-2xl shadow-md border-r-[${
            i % 2 !== 0 ? "#A4C3B2" : "#6B9080"
          }] border-r-8 mb-10 relative text-black`}
        >
          <h2 className="text-2xl mb-1 text-black">{entry.title}</h2>
          <p className="font-light text-xs mb-6 text-slate-400">{entry.date}</p>
