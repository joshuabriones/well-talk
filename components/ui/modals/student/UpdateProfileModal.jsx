			...
			<div className="modal" role="dialog">
				...
				>
					...
					<section className="flex flex-col items-center">
						<AvatarDisplay session={session} handleSelectPhoto={handleSelectPhoto} />

						{tabSelected === "Account Settings" && <AccountTab session={session} />}
				...
				<label className="modal-backdrop" htmlFor="my_modal_7" onClick={() => onClose()}>
					Close
				</label>
			</div>
		</>
...
const AvatarDisplay = ({ session, handleSelectPhoto }) => {
	return (
...
const SecurityTab = ({ passwords, handlePasswordChange, showInvalidPassword }) => {
	return (
		<section className="h-full w-full px-20 flex flex-col items-center">
			<section className="w-full flex flex-col gap-y-5 py-2">
				{/* Current Password */}
...
const PasswordInput = ({ label, password, handlePasswordChange, showInvalidPassword }) => {
