  ...
  const fetchReferralToken = async (id) => {
    if (userSession) {
      try {
        ...
        if (!response.ok) {
          console.error(
            `Error fetching referral token: ${response.statusText}`
          );
          return;
        }
        ...
        switch (user?.role) {
          case "student":
            const token = await fetchReferralToken(
              notification?.referral?.referralId
            );
            if (token) {
      ...
      switch (type) {
        ...
        case "referral":
          if (notification?.receiver?.id === user?.id) {
            text = (
              <span>
                Prof. <span className="font-bold">{senderName}</span> has
                referred you for an appointment for reason:{" "}
  ...
  return (
    <div>
      ...
      {appointmentDoneModal && (
        <AppointmentDone
          setShowModal={setAppointmentDoneModal}
          appointment={appointment}
        />
      )}
