...
const ChatWidget = () => {
  ...
  return (
    <div className="fixed bottom-8 right-4 md:right-8 lg:right-8 z-50">
      ...
      <div
        id="chatBox"
        className={`fixed bottom-28 right-4 md:right-8 lg:right-8 w-full max-w-xs sm:max-w-sm md:max-w-sm lg:max-w-sm h-[52vh] bg-white border-2 rounded-2xl shadow-lg flex flex-col transition-all duration-300 ${
          ...
          <div className="flex-1 overflow-y-auto p-3 text-white text-opacity-50 space-y-4 mt-4">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`flex ${
                  msg.senderId === loggedInUser.id ? "justify-end" : ""
                }`}
              >
                <div
