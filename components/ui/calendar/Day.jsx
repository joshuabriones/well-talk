...
function Day({ day, rowIndex }) {
  ...
  // 👇👇this hook is for modals: Set Event and Set Appointment
  ...
  useEffect(() => {
    const events = filteredEvents.filter(
      (evt) =>
        moment(evt.startTime).format("DD-MM-YY") === day.format("DD-MM-YY")
    );
    setDayEvents(events);
  }, [filteredEvents, day]);
  ...
  function getCurrentDayClass() {
    ...
    const currentMonth = dayjs(new Date(dayjs().year(), monthIndex, 1)).format(
      "MM"
    );
    if (today === currentDay) {
      return "day-class";
