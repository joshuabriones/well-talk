...
import { faBookmark, faCheck, faClock, faTrash, faXmark } from "@fortawesome/free-solid-svg-icons";
...
export default function EventModal() {
	...
	function handleSubmit(e) {
		const calendarEvent = {
			...
			startTime: moment(
				daySelected.format("YYYY-MM-DD") + " " + startDateTime,
				"YYYY-MM-DD HH:mm"
			).format("YYYY-MM-DDTHH:mm:ss"),
			endTime: moment(
				daySelected.format("YYYY-MM-DD") + " " + endDateTime,
				"YYYY-MM-DD HH:mm"
			).format("YYYY-MM-DDTHH:mm:ss"),
			label: selectedLabel,
			status: "Scheduled",
		...
		if (selectedEvent) {
			axios
				.put(
					"http://localhost:5108/api/CalendarEvents/UpdateCalendarEvent",
					calendarEvent,
					{ withCredentials: true }
				)
				.then(function (response) {
					return dispatchCallEvent({
						type: "update",
			...
			axios
				.post("http://localhost:5108/api/CalendarEvents/AddCalendarEvent", calendarEvent, {
					withCredentials: true,
				})
				.then(function (response) {
						...
						{selectedEvent && (
							<button
								...
								onClick={() => {
									...
									axios
										.delete(
											`http://localhost:5108/api/CalendarEvents/DeleteCalendarEvent/${selectedEvent.eventId}`,
											{ withCredentials: true }
										)
										.then(function (response) {
											return dispatchCallEvent({
												type: "delete",
												payload: savedEvents.filter(
													(evt) => evt.eventId !== response.data
												),
											});
										})
										.catch((err) => {});
									...
									>
										...
										{startTimeComponent && (
											<div className="start-time-container">
												{monthViewStartTimeSlots &&
													monthViewStartTimeSlots.map((time, index) => (
														<div
															key={index}
															className="start-time-content"
															onClick={() => {
																setEventStartTime(
																	time.format("h:mma")
																);
									...
									>
										...
										{endTimeComponent && (
											<div className="end-time-container">
												{monthViewSlotsEndTime &&
													monthViewSlotsEndTime.map((time, index) => (
														<div
															key={index}
															className="end-time-content"
															onClick={() => {
																setEventEndTime(
																	time.format("h:mma")
																);
								...
								{showDayViewCalendar && (
									...
									>
										...
										{endTimeComponent && (
											<div className="end-time-container">
												{timeSlotsEndTime &&
													timeSlotsEndTime.map((time, index) => (
														<div
															key={index}
															className="end-time-content"
															onClick={() => {
