...
function ProfileMenu() {
	...
	const handlePushToVerifyStudents = () => {
		if (userSession) {
			router.push(`/${userSession.role}/verifystudentaccounts`);
		}
	};
	...
	const profileMenuItems = [
		...
		},
		{
			...
			action: handlePushToVerifyStudents,
			visible: userSession?.role === "counselor",
		},
		{
			label: "Sign Out",
		...
		<div>
			...
			{isMenuOpen && (
				<div className="absolute right-12 w-36 bg-white border border-gray-300 rounded-lg shadow-lg font-Jaldi ">
					{profileMenuItems
						.filter((item) => item.visible !== false) // Filter out items not visible
	...
	switch (userType) {
		...
		case "counselor":
			navigationItems = [
				...
				{ label: "Dashboard", route: "/counselor/dashboard" },
				{
					label: "Appointments",
...
export function Navbar({ userType }) {
	const { showNotifications, setShowNotifications, unreadCount, setUnreadCount } =
		...
		>
			<div className="relative mx-auto flex items-center justify-between text-gray">
				...
				<div className="md:ml-24 flex flex-row items-center">
					...
					>
						<img
							...
							alt="WellTalk Logo"
							className="w-18 h-14 hidden md:block"
						/>
				...
				>
					...
					/>
					{unreadCount > 0 && (
						<span className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full text-xs px-1 z-10">
							{unreadCount}
						</span>
					)}
				</div>
