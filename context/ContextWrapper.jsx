...
import { getTimeSlots } from "@/components/ui/calendar/TimeSlots";
...
function formatStartTime(selectedEvent, showMonthViewCalendar) {
  ...
  return selectedEvent
    ...
    : showMonthViewCalendar && "currentTime";
}
...
export default function ContextWrapper(props) {
  ...
  const [dayViewEventStartTime, setDayViewEventStartTime] = useState(
    moment()
      .add(moment().minute() > 30 && 1, "hours")
      .minutes(moment().minute() <= 30 ? 30 : 0)
      .format("h:mma")
  ); // dayview event start time
  const [dayViewEventEndTime, setDayViewEventEndTime] = useState(
    moment(
      moment()
        .add(moment().minute() > 30 && 1, "hours")
        .minutes(moment().minute() <= 30 ? 30 : 0)
  ...
  const [showAppointmentModal, setShowAppointmentModal] = useState(false); // showing appointment modal
  const [selectedEvent, setSelectedEvent] = useState(null); //selected event for updating
  const [eventStartTime, setEventStartTime] = useState(
    currentStartTime(selectedEvent)
  ); //month view start time
  const [eventEndTime, setEventEndTime] = useState(
    currentEndTime(selectedEvent)
  ); // month view end time
  ...
  const [startDateTime, setStartDateTime] = useState(
    formatStartTime(selectedEvent, showMonthViewCalendar)
  ); // for setting the event start time in HH:MM format
  ...
  useEffect(() => {
    let updateStartTime =
      selectedEvent && moment(selectedEvent.startTime).format("h:mma");
    let updateEndTime =
      selectedEvent && moment(selectedEvent.endTime).format("h:mma");
    let updateStartDateTime =
      selectedEvent && moment(selectedEvent.startTime).format("HH:mm");
    let updateEndDateTime =
      selectedEvent && moment(selectedEvent.endTime).format("HH:mm");
    showMonthViewCalendar && selectedEvent
      ? setEventStartTime(updateStartTime)
      : setEventStartTime(
          moment()
            .add(moment().minute() > 30 && 1, "hours")
            .minutes(moment().minute() <= 30 ? 30 : 0)
    ...
    showMonthViewCalendar && selectedEvent
      ? setStartDateTime(updateStartDateTime)
      : setStartDateTime(
          moment()
            .add(moment().minute() > 30 && 1, "hours")
            .minutes(moment().minute() <= 30 ? 30 : 0)
    ...
    showMonthViewCalendar && selectedEvent
      ? setEndDateTime(updateEndDateTime)
      : setEndDateTime(
          moment(
            moment()
              .add(moment().minute() > 30 && 1, "hours")
    ...
    showDayViewCalendar && selectedEvent
      ? setDayViewEventStartTime(updateStartTime)
      : setDayViewEventStartTime(dayViewEventStartTime);
    showDayViewCalendar && selectedEvent
      ? setDayViewEventEndTime(updateEndTime)
      : setDayViewEventEndTime(dayViewEventEndTime);
    showDayViewCalendar && selectedEvent
      ? setStartDateTime(updateStartDateTime)
      : setDayViewEventStartTime(dayViewEventStartTime);
    showDayViewCalendar && selectedEvent
